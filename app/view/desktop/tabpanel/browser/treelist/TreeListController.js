...
Ext.define('FSS.view.desktop.tabpanel.browser.treelist.TreeListController', {
    ...
    control: {
        '[xtype=fssList]': {
            childtap: 'listSelectRouteHandler',
        }
    },
    ...
    /**
     * Sets active league, currently selected item
    ...
    setActiveLeague: function(league){
        if (!league) {
            ...
            if (Ext.isString(league)) {
                var subLeagues;
                var active = this.getActiveLeague().league;
                subLeagues = active ? active.subLeagues : undefined;
                league = {
                    league: league,
    ...
    getListIndex: function(){
        ...
        var league = this.getActiveLeague().league;
    ...
    getDbUrl: function(previousLeague, leagueId){
        ...
        if (urlTpl) { // none or one league selected
        ...
        if (leagueList) {
            ...
            if (Ext.isArray(leagues) && leagues.length) {
                ...
                if (!store.getCount() || previousActiveLeague !== activeLeague) {
                    ...
                    if (!store.findRecord('id', this.getActiveLeague().league)) {
                ...
                if (activeLeague) { // if there is selected item then show its details
                    this.fireEvent('e_loadDetails', activeLeague);
                }
            }
        }
    ...
    pullActiveLeague: function(leagueList){
        ...
        return activeLeague ? activeLeague : undefined;
    },
    ...
    findList: function(reference){
        ...
        if (!list) { // there can be nested list within collapsible panel
            var listPanel = this.lookup(reference + 'Panel');
            if (listPanel) {
                list = listPanel.lookup(reference);
            }
        }
        return list;
    },
     ...
     * @param {FSS.view.desktop.tabpanel.browser.treelist.list.List} list
     * @param {Ext.list.Location} listItem
     */
